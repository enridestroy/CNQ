...
cd apr-1.6.3 ; ./configure --quiet ; make --quiet ; make install ; cd 
sleep 5
wget -nc http://httpd-mirror.sergal.org/apache/apr/apr-util-1.6.1.tar.gz ; \
tar -zxvf apr-util-1.6.1.tar.gz ; \
cd apr-util-1.6.1 ; ./configure --quiet --with-apr=/usr/local/apr ; make --quiet; make install ; cd 
sleep 5
cd /opt/tomcat/bin
...
./configure --quiet --with-apr=/usr/local/apr --with-java-home=/usr/lib/jvm/java-1.7.0-openjdk-1.7.0.161-2.6.12.0.el7_4.x86_64
make --quiet
make install
